...
const ServiceCard = ({ service, isSelected, onSelect, color }) => {
  return (
    ...
    >
      ...
      <p className={`text-${color}-100/80 mb-4`}>{service.description}</p>

      <ul className={`space-y-2 text-${color}-100/90 text-sm`}>
        ...
        <AnimatePresence>
          {selectedServices.size > 0 && (
            <motion.div
              ...
              className="mt-12 p-6 bg-white/5 backdrop-blur-lg rounded-xl border border-teal-400/20 text-center"
            >
              <button className="bg-teal-400 hover:bg-teal-300 text-teal-900 px-6 py-3 rounded-full font-medium">
