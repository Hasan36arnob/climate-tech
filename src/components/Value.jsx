    ...
    <div id="value">
      <section className="py-24 bg-gradient-to-br from-green-800 via-green-700 to-green-900 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          {/* Header Section */}
          <div className="text-center mb-20 text-white-600">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-20">
            {valueProps.map((prop, index) => (
              <div
                key={index}
                className="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-green-400/10 hover:border-green-400/20 transition-all duration-300 hover:transform hover:-translate-y-1"
              >
                <div className="bg-green-400/10 rounded-xl p-3 inline-block mb-6">
                  {prop.icon}
                </div>
                <h3 className="text-2xl font-semibold text-white mb-4">
                  {prop.title}
                </h3>
                <p className="text-green-100/70">{prop.description}</p>
              </div>
            ))}
          </div>
